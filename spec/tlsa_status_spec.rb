...
RSpec.describe InternetSecurityEvent::TLSAStatus do
  subject do
    InternetSecurityEvent::TLSAStatus.new(record, certificate)
  end
  ...
  let(:certificate) do
    OpenSSL::X509::Certificate.new(File.read('spec/mx.blogreen.org.crt'))
  end
  ...
  context '#certificate_association_data' do
    it 'computes correct checksums' do
      expect(subject.certificate_association_data(0, 0)).to eq('3082066930820551a0030201020212035107c642a503567ecfdce5fdc79d1b54903' \
                                                               ...
                                                               '5f1373f')
      expect(subject.certificate_association_data(0, 1)).to eq('015ad9a7cb61431733b483cd7e155f38f7a776fa0ef7f0ed94da3ca8d86c210a')
      expect(subject.certificate_association_data(0, 2)).to eq('e9278bf56a108dbce0e32dc2366dbb879fb9f3baaaa5c8835c93aec6d5bde2a' \
                                                               '1b1bc1606347c8b4954b12ff63f730eedcd2d345a08b64e18f0056a6e38ab07' \
                                                               'ba')
      expect(subject.certificate_association_data(1, 0)).to eq('30820222300d06092a864886f70d01010105000382020f003082020a0282020100c' \
      ...
      expect(subject.certificate_association_data(1, 1)).to eq('d8aac0d602e5532136ffb9e368fbc3c9a7a4b694340800b08731bcc09099a925')
      expect(subject.certificate_association_data(1, 2)).to eq('a7840b0dfc18056727b2bfc1f66f2d6fff91ef8295ea9956b5954fcd3e5c4c8' \
